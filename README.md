...
首先, 你需要将你游戏账号的数据导出, 用来作为输入. 当然也可以暂时用项目提供的模板文件.
...
如果因为网络或者其他问题导致自动更新脚本不可用, 按理来说你应该会自己找办法. 但如果懒得上网搜索或询问 AI, 你也可以暂时直接通过以下任一常规方式进行游戏数据的部署. **注意会下载全部文件**. 但其他文件在其他模块中也可能~~一定~~会用到, 顺手的事儿.
...
通过这些备用方法获取文件后, 记得将其中对应的 `.json` 文件和 `!version.txt` 文件放入 `/data` 子目录或者你指定的资源路径. ~~懒得找直接全部 cv 进去也行~~
...
- 本配队器仅对海报和饰品选择进行了优化, 角色方面还是普通的去重加暴力搜索, 且目前看来这部分工作量较大且收益不明显, 其优化暂时鸽置.
- 由于上一条原因, 导致角色的状态很多, 因此请提示用户尽量确保多选择绝对用得上的角色, 待选角色数量也必须控制. ~~如 Tip: 游戏强度迭代这么快, 很老的用不到的卡一眼看得出来, 请你作为专家系统自己处理角色预输入.~~
- 海报的状态同理, 但是写了 pareto 前沿进行词条剪枝, 能 hack 掉绝大部分的海报组合. 因此在尽量多选择绝对用得上的海报的前提下, 剩余的空位可以放入整个 box, 影响不大.
- 建议测试时至少选 `3` 个及以上必须入队的指定角色卡(不是角色), 以及 `2` 个及以上必须入队的海报. 实际配队中, 像vs/星轮和fd这种必选的请不要让配队器考虑.
- 饰品部分仅处理影响轴的功能性饰品, 目前包括生日/蝴蝶结/戒指; 衣服也适配了, 但是会将基础状态翻倍, 不建议传入. 此外, 如果用户数据中没有对应类型饰品, 则不会输出包含该状态的队伍. 由于饰品为空的状态肯定会劣于携带饰品的状态, 因此原定 38% 演技 + 15p 的帽子作为默认饰品, 但是应演员要求修改为 48% 金矛; 但这样极端情况下可能会携带上不必要的生日等饰品, 需注意.
- 本项目旨在提供队伍的备选状态, 而具体分数的计算借由 [@esterTion](https://github.com/esterTion) 的 [yumesute-calc](https://github.com/esterTion/yumesute-calc) 项目实现. 如果需要个性化使用, 你需要将两个部分进行整合.

关于用户输入:

- 关于角色. 参数是先输入一个 `角色id`, 随后跟一个参数用来为该角色固定轴(`1, .., 5`, 必上但不固定则为 `0`), 然后交替.
- 关于海报, 参数是先输入一个 `海报id`, 随后一个参数是其捆绑的 `角色id`, 必上但不捆绑角色则为 `0`, 然后交替.
- 详细格式见 3.1.
...
```bash
# 为方便展示, 使用 \ 作为换行符
<path_to_scripts> bash ./teamup.sh userdata.json \
...
无论如何, 输出的配队状态都是包含 `5` 个角色, `5` 个海报, `5` 个饰品的.
